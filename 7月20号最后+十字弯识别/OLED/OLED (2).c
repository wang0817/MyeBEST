...
#define OLED_SCL PORTE,5  
#define OLED_SDA PORTE,1
#define OLED_RST PORTE,3 
#define OLED_DC  PORTE,2 
...
void OLED_Init(void)        
{
  //DDRM=0XFF;
        ...
        GPIO_SetPinDir(OLED_RST,1);//复位低能电平
...
void Dis_Num(byte y, byte x, unsigned int num,byte num_bit) 
{
    signed   char i;
    unsigned char ii;
    unsigned char dat[6];
    ...
    for(i = 0; i < num_bit; i++)
    OLED_P6x8Str(x + i, y, ' ');			//清显示区域
    for(i = ii; i >= 0; i--)
    OLED_P6x8Str(x++, y, dat[i]);			//输出数值}
}
